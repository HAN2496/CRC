...
class GP:
    def __init__(self, subject, path='gaussian_process_regressor.joblib'):
        ...
        self.X_time_original = self.X_time.copy()
    ...
    def predict_by_heelstrike(self, start=0, end=100, save_data=True): #scalar 값으로 predict 계산시 사용
        ...
        return self.predict(X, save_data=save_data)
    ...
    def scale(self, scale_x=1.0, scale_y=1.0, x_pos=0, y_pos=0, save_data=False, idx=-1):
        ...
        if x_pos != 0:
            ...
            X_scalar_gp = self.X_scalar * scale_x
        if y_pos != 0:
            y_pred_gp = self.y_pred - y_pos
            y_pred_gp *= scale_y
            y_pred_gp += y_pos
        else:
            y_pred_gp = self.y_pred * scale_y

        # if scale_x != 1.0:
        #     interpolator = interp1d(X_scalar_gp, y_pred_gp, kind='linear', bounds_error=False, fill_value="extrapolate")
        #     y_pred_gp = interpolator(self.X_time_original)
        ...
        #         cutted_datas.append([X_scalar_gp[idxs], y_pred_gp[idxs], self.X_scalar_original[idxs]])
        ...
        # plt.plot(self.X_time, self.y_pred_original, color='b')
        # plt.plot(self.X_time, y_pred_gp, color='r')
        # plt.show()
