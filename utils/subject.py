...
class Subject:
    def __init__(self, number, data_folder, subjects_list, is_hip, choose_one_dataset=True, extract_walking=True):
    ...
    def process_hip_sagittal(self, df):
        ...
        self.datas.append('hip_sagittal_torque',  torque)

    def process_knee_sagittal(self, df):
        knee_sagittal = df['knee_sagittal'].values
        pos, vel, acc = self.calc_kinematics(knee_sagittal)
        torque = self.inertia * acc
        self.datas.append('knee_sagittal', pos)
        self.datas.append('knee_sagittal_v', vel)
        self.datas.append('knee_sagittal_a',  acc)
        self.datas.append('knee_sagittal_torque',  torque)
