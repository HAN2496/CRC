...
class TD(Subject):
    def __init__(self, number, is_hip=True, choose_one_dataset=True, extract_walking=True):
        super().__init__(number, TD_FOLDER, TD_SUBJECTS, is_hip, extract_walking=extract_walking)
    ...
    def load_data(self):
        ...
        for filename in os.listdir(heelstrike_path):
            if filename.endswith('.csv'):
                ...
                self.process_knee_sagittal(df_gon[::5])
    ...
    def extract_normal_walking_datasets(self, num=None):
        ...
        if num is not None:
            return filtered_file_idx[num]
        else:
            return filtered_file_idx[np.random.randint(0, filtered_file_len)]

    def extract_one_datasets(self, extract_normal_phase=False, num=None):
        datas = Datasets()
        file_num = len(self.datas['file_names'])
        if extract_normal_phase:
            self.random_data_num = self.extract_normal_walking_datasets(num=num)
        else:
