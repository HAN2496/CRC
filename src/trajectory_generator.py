...
class Reference:
    def __init__(self, fit_mode=False, X=None, y=None, filename='models/gaussian_process_regressor'):
        if fit_mode:
            if X is None or y is None:
                raise ValueError("X and y must be provided if fit_mode is True")
            self.fit(X, y, filename)
        self.model = load(f'{filename}.joblib')

    def fit(self, heelstrike, y, filename):
