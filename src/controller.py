...
from configs.config_control import Kp, Kd, dt
...
class Controller:
    ...
    def control(self, control_target, subject_datas, torques):
        control_target_v = f"{control_target}_v"
        scale_x, scale_y = self.minimize_scale(subject_datas['heelstrike'][-1], subject_datas['header'], subject_datas[control_target])
        ...
        error_before = y_reference[1] - subject_datas[control_target][-1]
        x_t = subject_datas[control_target][-1]
        v_t = subject_datas[control_target_v][-1]
        # print(f"scale: {scale_x, scale_y}")
        v_reference = np.diff(y_reference[1:self.control_interval+2]) / dt

        desired_kinematics = []

        for i in range(self.control_interval):
            error =  y_reference[i+1] - x_t
            d_error = v_reference[i] - subject_datas[control_target_v][-1]
