...
def divide_by_section(data):
    heel_strike_indices = []
    for entry in data:
        heel_strike_indices = find_zero_indices(entry['heel_strike'], 0)
        heel_strike_indices.append(heel_strike_indices)
...
def index_by_scalar(data, heel_strike_indices, start=0, end=90, index_pos=4):
    start, end = int(start), int(end)
    random_test_num = np.random.randint(0, len(data))
    random_test_num = 0
    entry = data[random_test_num]
    start_idx = heel_strike_indices[random_test_num][4]
...
class GP:
    ...
    def translation(self, delx, dely):
        self.X_scalar -= delx
        self.y_pred += dely
