...
class Control:
    ...
    def control(self, idx):
        ...
        if idx < self.collect_num:
            ...
            self.gp.translation(delx=0, dely=y_pred - self.corrected_datas['hip_sagittal'][-1])
            ...
            _, y_pred_gp = self.gp.scale(x_scale, y_scale,
                                                   self.corrected_datas['heelstrike'][-1], self.corrected_datas['hip_sagittal'][-1])

            torque_gp = self.subject.calc_torque(y_pred_gp[:idx])
            torque_subject = self.original_datas['torque'][idx]
            error = y_pred_gp[idx+1] - y_pred_gp[idx]
            torque_input = self.Kp * error + torque_subject

            a_t = self.subject.move(torque_input=torque_input)[0]
            a_dt = a_t - self.corrected_datas['hip_sagittal_acc'][-1]
